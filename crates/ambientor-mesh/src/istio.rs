...
pub async fn collect_istio_policies(client: &Client) -> anyhow::Result<IstioPolicyObjects> {
    ...
    Ok(IstioPolicyObjects {
        peer_authentications: list_namespaced_cr(client, &peer_ar).await.unwrap_or_default(),
        authorization_policies: list_namespaced_cr(client, &auth_ar).await.unwrap_or_default(),
        ...
        wasm_plugins: list_namespaced_cr(client, &wasm_ar).await.unwrap_or_default(),
